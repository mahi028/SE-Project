...
class CreateGroup(graphene.Mutation):
    ...
    def mutate(self, info, label, timing, pincode=None, location=None):
        # Get the authenticated senior
        senior = get_senior(info)
        if not senior:
            return ReturnType(message="Authentication required", status=403)

        try:
            # Handle timing without UTC conversion
            if isinstance(timing, str):
                # Parse the datetime string preserving local time
                from dateutil import parser as date_parser
                group_datetime = date_parser.parse(timing)

                # If timezone info is present, convert to naive datetime for storage
                if group_datetime.tzinfo is not None:
                    # Store as UTC but preserve the intended local time
                    group_datetime_for_db = datetime(*group_datetime.utctimetuple()[:6])
                else:
                    # If no timezone info, use as-is (assumes local time)
                    group_datetime_for_db = group_datetime
            else:
                group_datetime_for_db = timing

            print(f"Creating group with timing: {group_datetime_for_db}")

            # Create new group
            new_group = Group(
                label=label,
                timing=group_datetime_for_db,
                admin=senior.sen_id,
                pincode=pincode or senior.pincode,
                location=location
            )
            ...
            return ReturnType(
                message=f"Peer group '{label}' has been created successfully",
                status=1
            )
