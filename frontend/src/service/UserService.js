export const userService = {
    getUsers(){
        return [
            // Doctors
            {
                "ez_id": "D001",
                "role": "doctor",
                "email": "dr.rajesh@citycarehospital.com"
            },
            {
                "ez_id": "D002",
                "role": "doctor",
                "email": "dr.priya@greenvalleyclinic.in"
            },
            {
                "ez_id": "D003",
                "role": "doctor",
                "email": "dr.amit@sunrisehospitals.com"
            },
            {
                "ez_id": "D004",
                "role": "doctor",
                "email": "dr.sunita@lifeline.com"
            },
            {
                "ez_id": "D005",
                "role": "doctor",
                "email": "dr.vikram@hopewellmed.com"
            },
            {
                "ez_id": "D006",
                "role": "doctor",
                "email": "dr.kavita@delhiskin.com"
            },
            {
                "ez_id": "D007",
                "role": "doctor",
                "email": "dr.rohit@fortunehospital.com"
            },
            {
                "ez_id": "D008",
                "role": "doctor",
                "email": "dr.meera@noidaclinic.com"
            },
            {
                "ez_id": "D009",
                "role": "doctor",
                "email": "dr.anuj@metrocenter.com"
            },
            {
                "ez_id": "D010",
                "role": "doctor",
                "email": "dr.pooja@healcare.org"
            },
            {
                "ez_id": "D011",
                "role": "doctor",
                "email": "dr.deepak@wellness.com"
            },
            {
                "ez_id": "D012",
                "role": "doctor",
                "email": "dr.nisha@noidawomen.com"
            },
            {
                "ez_id": "D013",
                "role": "doctor",
                "email": "dr.suresh@ghzbmed.com"
            },
            {
                "ez_id": "D014",
                "role": "doctor",
                "email": "dr.rekha@vivek.com"
            },
            {
                "ez_id": "D015",
                "role": "doctor",
                "email": "dr.manoj@sarvoday.org"
            },
            {
                "ez_id": "D016",
                "role": "doctor",
                "email": "dr.sanjay@khannaclinic.in"
            },
            {
                "ez_id": "D017",
                "role": "doctor",
                "email": "dr.smita@carecure.in"
            },
            {
                "ez_id": "D018",
                "role": "doctor",
                "email": "dr.anil@ghzhealth.com"
            },
            {
                "ez_id": "D019",
                "role": "doctor",
                "email": "dr.rakesh@bonecare.com"
            },
            {
                "ez_id": "D020",
                "role": "doctor",
                "email": "dr.neetu@kidscare.com"
            },
            {
                "ez_id": "D021",
                "role": "doctor",
                "email": "dr.ashok@skinsolutions.com"
            },
            {
                "ez_id": "D022",
                "role": "doctor",
                "email": "dr.vandana@womenshealth.com"
            },
            {
                "ez_id": "D023",
                "role": "doctor",
                "email": "dr.ravi@mindcare.com"
            },
            {
                "ez_id": "D024",
                "role": "doctor",
                "email": "dr.preeti@eyecare.com"
            },
            {
                "ez_id": "D025",
                "role": "doctor",
                "email": "dr.hemant@nirmanmed.in"
            },
            {
                "ez_id": "D026",
                "role": "doctor",
                "email": "dr.arun@superrohini.com"
            },
            {
                "ez_id": "D027",
                "role": "doctor",
                "email": "dr.seema@sunbeam.com"
            },
            {
                "ez_id": "D028",
                "role": "doctor",
                "email": "dr.rahul@clinicplus.in"
            },
            {
                "ez_id": "D029",
                "role": "doctor",
                "email": "dr.shilpa@familycare.org"
            },
            {
                "ez_id": "D030",
                "role": "doctor",
                "email": "dr.ajay@eastdelhi.com"
            },
            {
                "ez_id": "D031",
                "role": "doctor",
                "email": "dr.sushma@yamunapoly.com"
            },
            {
                "ez_id": "D032",
                "role": "doctor",
                "email": "dr.vinod@swasthya.com"
            },
            {
                "ez_id": "D033",
                "role": "doctor",
                "email": "dr.neha@heartpoint.com"
            },
            {
                "ez_id": "D034",
                "role": "doctor",
                "email": "dr.ramesh@shakticlinic.org"
            },

            // Seniors
            {
                "ez_id": "S001",
                "role": "senior",
                "email": "rajesh.kumar@gmail.com"
            },
            {
                "ez_id": "S002",
                "role": "senior",
                "email": "sunita.sharma@yahoo.com"
            },
            {
                "ez_id": "S003",
                "role": "senior",
                "email": "vikram.gupta@hotmail.com"
            },
            {
                "ez_id": "S004",
                "role": "senior",
                "email": "kamala.devi@gmail.com"
            },
            {
                "ez_id": "S005",
                "role": "senior",
                "email": "ramesh.verma@rediffmail.com"
            },
            {
                "ez_id": "S006",
                "role": "senior",
                "email": "lata.khanna@gmail.com"
            },
            {
                "ez_id": "S007",
                "role": "senior",
                "email": "ashok.malhotra@outlook.com"
            },
            {
                "ez_id": "S008",
                "role": "senior",
                "email": "usha.kapoor@yahoo.in"
            },
            {
                "ez_id": "S009",
                "role": "senior",
                "email": "mohan.sharma@gmail.com"
            },
            {
                "ez_id": "S010",
                "role": "senior",
                "email": "shakuntala.rani@rediffmail.com"
            }
        ]
    },

    getUser(query) {
        if (query['email']){
            return Promise.resolve(
                this.getUsers().find(user => user["email"] === query['email'])
            );
        }
        else if (query['ez_id']){
            return Promise.resolve(
                this.getUsers().find(user => user["ez_id"] === query['ez_id'])
            );
        }
        return Promise.resolve(null);
    },
}
